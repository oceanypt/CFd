...
def generate_batch(batch):
	...
	# text (seq, batch, embed)
	#text = pad_sequence([ entry[1].squeeze() for entry in batch ], batch_first = True)
...
def train_func_MI(params, lstm,  large, fc, domain_fc, criterion, optimizer,  raw_train_set, train_set, teach_lstm = None, teach_fc = None):
	...
	for (text1, cls1), (text2, cls2), (text3, cls3), (text4, cls4) in zip(data[0], data[1], source_labeled_data, raw_set):
		...
		if params.mi_lamda_t != 0 and params.mi_lamda_s != 0:
			...
			#ids = torch.range(start = 0, end = z2.size(0)-1).to(device)
			for i in range(neg_n):
				r = torch.randperm(z2.size(0))
				z_z_ave_2_shuffle_score.append(cos(large[0](z2), z_ave_2[r]).view(-1, 1))
			...
			if neg_n == 0:
				local_loss = -torch.mean(z_z_ave_2_score) * params.mi_lamda_t
	...
	return train_loss / len(train_set), domain_loss / len(train_set), output_cluster_loss.item()
...
def train(params):
	...
	#fc = FC_all(params.embed_size, params.hidden_size, params.nclass).to(device)
...
def train_MI(params, teach_lstm = None, teach_fc = None):
	...
	#fc = Anchor(params.hidden_size, params.nclass).to(device)

	#lstm, global_dis = initialize_MI(params)
	lstm = Ave(params.embed_size, params.hidden_size).to(device)
	#fc_d = initialize_domain(params)
	...
	params.num_to_return = 950
	do_pesudo = params.pseudo_t_lamda
	for epoch in range(n_epochs):
		start_time = time.time()
		
		params.num_to_return += 100 
		if do_pesudo:
		...
		if lstm_best == None:	
			...
			train_loss, domain_loss, cluster_loss  = train_func_MI(params, lstm, large, fc, domain_fc, criterion, optimizer,  raw_train_set, train_set, lstm_best, fc_best)


		all_cluster_loss.append(cluster_loss)
		...
		if best_valid_loss > valid_loss:
			...
			#params, lstm, global_dis, path
			#save(params, lstm,  params.save_mi_path)
...
if __name__ == '__main__':
	...
	for i in range(len(raw_train_set)):   #params.ndomain):
		c_raw_train_set = raw_train_set[i]
		for j in range(len(c_raw_train_set)):
			c_raw_train_set[j][0] = float(i)
		raw_train_set[i] = c_raw_train_set
